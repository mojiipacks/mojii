  ...
  return (
    <div className="min-h-screen">
      <div className="px-6 pt-32 pb-6 max-w-7xl mx-auto">
        <Link href={`/${lang}/#packs`} className="text-gray-dim text-sm tracking-widest uppercase hover:text-green-electric transition-colors">
          {t.pack.back}
        </Link>
      </div>
      ...
      <section className="px-6 pb-16 max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 gap-12 items-end">
          <div>
            <p className="text-green-electric text-sm tracking-[0.3em] uppercase mb-3">{pack.genre}</p>
            <h1 className="text-7xl md:text-9xl text-white leading-none mb-4" style={{ fontFamily: 'Bebas Neue, sans-serif' }}>
              MOJII<br />
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={item.label} className="flex justify-between items-center py-3 border-b border-gray-border">
  ...
  <section className="px-6 pb-16 max-w-7xl mx-auto">
    ...
    />
      ...
      <section className="px-6 pb-24 max-w-7xl mx-auto">
        <p className="text-green-electric text-sm tracking-[0.3em] uppercase mb-8">{t.pack.chooseTier}</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {pack.tiers.map((tier, i) => (
            <TierCard key={tier.id} tier={tier} packTitle={pack.title} highlighted={i === 1} t={t.pack} lang={lang} />
          ))}
        </div>
      ...
      <section className="px-6 pb-16 max-w-7xl mx-auto">
        <div className="flex flex-wrap gap-2">
          {pack.tags.map((tag) => (
            <span key={tag} className="text-xs text-gray-dim border border-gray-border px-3 py-1 rounded-full uppercase tracking-widest">
              {tag}
            </span>
          ))}
        </div>

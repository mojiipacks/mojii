...
const content = {
  en: {
    ...
    updated: `Effective for all MOJII purchases. Last updated: ${new Date().getFullYear()}.`,
    sections: [
      {
  ...
  uk: {
    ...
    updated: `Діє для всіх покупок MOJII. Останнє оновлення: ${new Date().getFullYear()}.`,
    sections: [
      {
                ...
                <ul className="space-y-2">
                  {s.list.map((item) => (
                    <li key={item} className="flex items-start gap-3">
                      <span
                        className={`mt-1 shrink-0 ${s.type === "allow" ? "text-green-electric" : "text-red-400"}`}
                      >
                        {s.type === "allow" ? "✓" : "✕"}
                      </span>
                      {item}
